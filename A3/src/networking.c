...
void register_user(char* username, char* password, char* salt)
{
    ...
    RequestHeader_t header = request.header;

    compsys_helper_readinitb(&state, network_socket);

    if (compsys_helper_writen(network_socket, &header, sizeof(header)) != sizeof(header)) {
        perror("Write");
        exit(1);
    }
    ...
    if ( length > 0) {
        ...
        message[length] = '\0';
        printf("Got response of %d bytes: %s\n", length, message);
    }
...
void get_file(char* username, char* password, char* salt, char* to_get, uint32_t len)
{
    ...
    uint32_t block_count = ntohl(response_header->blockCount);
    uint32_t code = ntohl(response_header->statusCode);

    if (block_count > 0 && code == 1) {

        // Initialize payload array
        int32_t total_len = 0;
        char *array[block_count];
